 ...
 * @Author: huazj
 * @Date: 2022-02-05 10:53:47
 * @LastEditTime: 2022-02-05 12:47:29
 * @LastEditors: huazj
 */
//  加强版防抖 防抖+节流
...
/**
 * @description: 防抖函数：函数被触发 n 秒后在执行回调，如果在这 n 秒内又被触发，则重新计时
 * @param  {*}
 * @return {*}
 * @param {Function} fn 要执行的函数
 * @param {Number} wait wait毫秒后执行 
 * @param {array} params1 传递给fn的参数
 */
function debounce(fn, wait, ...params1) {
    ...
    return function (...params2) {
        ...
        // 设置定时器/重置定时器
        timer = setTimeout(() => {
            // wait时间后 执行回调 期间在触发debounce 需要重新等待
...
const cb = debounce(actionFn, 500, 'actionFn参数', '参数2');
setInterval(cb, 1000);//第一次在1500ms后触发，之后每1000ms触发一次
setInterval(debounce(actionFn, 2000), 1000);//note：还没执行就一直重复触发，不会执行，所以需要节流
